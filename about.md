---
layout: page
title: About & Contact
permalink: /about/
hide_chrome: true
---

## Jean Pougoue — Network & System Engineer -> Cloud Security

**Transitioning from infrastructure and network engineering to Hybrid Cloud Security Engineering.**

---

## Current Role

Network & System Engineer at **KCP Network**, managing infrastructure across 70 hotel sites
supporting 3,500+ users.

## Professional Background

- 5+ years in network infrastructure and cloud migration
- Microsoft 365 and Entra ID administration
- Led migration of 200+ AccorInvest properties to Cisco Meraki
- Enterprise networking and security implementations

## Current Certifications

- ✅ Microsoft AZ-900 (Azure Fundamentals)
- ✅ Microsoft MS-900 (Microsoft 365 Fundamentals)
- ✅ NCFE Level 3 Certificate in Cyber Security
- 🔄 Cisco CCNA (renewal in progress)

## Home Lab

- **Hypervisor:** Proxmox on Fujitsu Esprimo G9010
- **Networking:** HP Aruba switches with VLAN segmentation
- **Security:** OPNsense firewall with advanced policies
- **Cloud:** Azure free tier for testing and learning

## About This Blog

Documenting the systematic transition from network engineering to cloud security engineering.
Every study session, practice test, certification attempt, and automation project is shared
transparently to help others follow a similar path.

---

## Get In Touch

Always interested in connecting with professionals in cybersecurity, cloud engineering,
and network infrastructure.

**Email:** [jdd.pougoue@gmail.com](mailto:jdd.pougoue@gmail.com)  
**LinkedIn:** [Jean Pougoue](https://linkedin.com/in/jean-pougoue)  
**GitHub:** [k3n-p4-chi](https://github.com/k3n-p4-chi)  
**Location:** South Ockendon, UK - available for events in the Greater London area

## What I'd Like to Discuss

- Career transitions into cybersecurity
- Azure and cloud security best practices
- Automation projects and scripting
- Home lab configurations and networking
- Certification study strategies and resources

## Collaboration

- Guest posting on relevant topics
- Technical reviews of automation scripts
- Study group participation for certification preparation
- Professional mentoring (both directions)

---

*I typically respond to professional enquiries within 24-48 hours.*
